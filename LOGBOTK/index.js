		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '⚠ KKK, só o dono pode usar isso poh KKKK',
					ownerB: ' KKK, só o dono pode usar isso poh KKKK ',
					admin: '⚠Só adm pode usar esse comando garai KKK',
			...
			const ownerNumber = ["553898456180@s.whatsapp.net", "994406695196@s.whatsapp.net", "554999929075@s.whatsapp.net", "15803133703@s.whatsapp.net", "5511954046176@s.whatsapp.net", "557186276526@s.whatsapp.net", "556299190316@s.whatsapp.net", "553898456180@s.whatsapp.net", "17577505877@s.whatsapp.net"] // replace this with your number
			...
			switch (command) {
				...
				case 'ts':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						reply('Transmissão enviada caraikjk')
					}
					break
				case 'promote':
					if (isOn == false) return reply(mess.error.isOff)
				...
				case 'kick':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length < 2) {
						client.sendMessage(from, `@${mentioned[0].split('@')[0]} levou ban o otario(a)`, text)
				...
				case 'creditos':
					if (isOn == false) return reply(mess.error.isOff)
					if (true) return reply('Creditos: eterno chollo \n eu https://wa.me/553898456180', text)
					break
				...
				case 'lista':
					try {
						if (!isGroup) return reply(mess.only.group)
						d = []
						teks = 'TOP 5 MAIS GADOS DO GRUPO KKK: \n'
						for (i = 0; i < 5; i++) {
				...
				case 'ytmsc':
					try {
						...
						client.sendMessage(from, lagu, audio, { mimetype: 'audio/mp4', filename: `${anu.title}byLOG.mp3`, quoted: mek, ptt: true })
						client.sendMessage(from, "LOG\n\nEnviando informações...", text, { quoted: mek })
					}
					catch (e) {
						reply("Error, tente novamente mais tarde")
						console.log("Error: " + e)
